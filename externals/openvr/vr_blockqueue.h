...
public:
  ...
  ReadPathBatch(PropertyContainerHandle_t ulRootHandle, PathRead_t *pBatch,
                uint32_t unBatchEntryCount) = 0;
  virtual ETrackedPropertyError
  WritePathBatch(PropertyContainerHandle_t ulRootHandle, PathWrite_t *pBatch,
                 uint32_t unBatchEntryCount) = 0;
  virtual ETrackedPropertyError StringToHandle(PathHandle_t *pHandle,
...
public:
  virtual EBlockQueueError Create(PropertyContainerHandle_t *pulQueueHandle,
                                  const char *pchPath, uint32_t unBlockDataSize,
                                  uint32_t unBlockHeaderSize,
                                  uint32_t unBlockCount, uint32_t unFlags) = 0;

  virtual EBlockQueueError Connect(PropertyContainerHandle_t *pulQueueHandle,
                                   const char *pchPath) = 0;
  ...
  virtual EBlockQueueError
  ReleaseWriteOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                        PropertyContainerHandle_t ulBlockHandle) = 0;

  virtual EBlockQueueError
  WaitAndAcquireReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                              PropertyContainerHandle_t *pulBlockHandle,
                              void **ppvBuffer, EBlockQueueReadType eReadType,
                              uint32_t unTimeoutMs) = 0;

  virtual EBlockQueueError
  AcquireReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                       PropertyContainerHandle_t *pulBlockHandle,
                       void **ppvBuffer, EBlockQueueReadType eReadType) = 0;

  virtual EBlockQueueError
  ReleaseReadOnlyBlock(PropertyContainerHandle_t ulQueueHandle,
                       PropertyContainerHandle_t ulBlockHandle) = 0;

  virtual EBlockQueueError
  QueueHasReader(PropertyContainerHandle_t ulQueueHandle,
                 bool *pbHasReaders) = 0;
};
